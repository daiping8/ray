...
void RuntimeOperations::Initialize(const RuntimeInitializeOptions& options) {
  ...
  ray::gcs::GcsClientOptions gcs_options(gcs_server_address, cluster_id,
  ...
  core_options.install_failure_signal_handler =
  ...
  g_worker_mode.store(static_cast<int>(options.worker_type),
...
ray::rpc::WorkerType RuntimeOperations::GetWorkerMode() const {
  return static_cast<ray::rpc::WorkerType>(
      g_worker_mode.load(std::memory_order_relaxed));
}
...
std::shared_ptr<ray::RayObject> RuntimeOperations::AllocateObject(
  ...
  auto go_buffer = AllocateGoHeapBuffer(
      reinterpret_cast<const char*>(object_id.Data()),
  ...
  if (!go_buffer) {
    RAY_LOG(ERROR) << "Failed to allocate Go heap buffer for object "
                   << object_id.Hex();
    return nullptr;
  }
...
void RuntimeOperations::InitializeRayLogging(
    const RuntimeInitializeOptions& options) {
...
void RuntimeOperations::InitializeCoreWorker(
    const RuntimeInitializeOptions& options) {
  RAY_CHECK(core_worker_ == nullptr)
      << "CoreWorker already initialized";
  ...
  RAY_CHECK(core_worker_ != nullptr)
      << "Failed to initialize CoreWorker";
}

void RuntimeOperations::ShutdownCoreWorker() {
  if (core_worker_) {
    // For driver mode, we should call Disconnect() before Shutdown() to match Java behavior.
    // This ensures the raylet is properly notified before the worker shuts down.
    if (g_worker_mode.load(std::memory_order_relaxed) == static_cast<int>(ray::rpc::WorkerType::DRIVER)) {
