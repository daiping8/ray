...
def install_stubs():
    ...
    services_module.get_cached_node_ip_address = lambda session_dir: "127.0.0.1"
    utils_module = types.ModuleType("ray._private.utils")
    logging_utils_module = types.ModuleType("ray._private.logging_utils")
    logging_utils_module.redirect_stdout_stderr_if_needed = lambda *args, **kwargs: None
...
def main():
    if len(sys.argv) != 3:
        raise SystemExit(
            f"usage: {pathlib.Path(sys.argv[0]).name} <reference_py> <logs_dir>"
        )

    reference_path = sys.argv[1]
    logs_dir = pathlib.Path(sys.argv[2])
    ...
    print(json.dumps(client.batches, indent=2, sort_keys=True))
